...
import com.example.todoLogBack.repositories.UserRepository;
import com.example.todoLogBack.services.UserService;
import com.example.todoLogBack.validators.UserValidator;
...
public class UserServiceImpl implements UserService {
    ...
    public UserDto login(UserDto user) {
        ...
        return userRepository.findUserByEmailAndPassword(user.getEmail(), user.getPassword())
                .map(UserDto::fromEntity)
                .orElseThrow(() -> new EntityNotFoundException("No user found with Email = " + user.getEmail()
                        + " and Password = <HIDDEN_PASSWORD>", ErrorCodes.USER_NOT_FOUND));
    }
}
